...
class InteractiveWindow(QWidget):
    ...
    def processImage(self):
        ...
        self.newPictureName = processMethod(self.fname)
        # print(self.newPictureName)
        self.picture.setPixmap(QPixmap(self.newPictureName))

    def fftImageProcess(self):
        self.newPictureName = f"./images/{self.fname.split('/')[-1].split('.')[0]}.svg"
        fileName = f"./images/{self.fname.split('/')[-1].split('.')[0]}.txt"

        if self.fname.split(".")[-1] != "svg":
            bitmap_to_contour_svg(self.fname, self.newPictureName)
        else:
            self.newPictureName = self.fname
        self.picture.setPixmap(QPixmap(self.newPictureName))
        # fftProcess(fileName)
        draw(self.newPictureName)
